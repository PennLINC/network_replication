...
make_connMetricsDemog_covbat <- function(atlas_network, metric, demog_csv, subj_id, dataset){
  subxparcel.matrix <- read.csv(sprintf("/cbica/projects/network_replication/output/%1$s/sensitivity_analyses/%2$s/%2$s_subxparcel_matrix_%3$s_covbat.csv", dataset, metric, atlas_network))
  print(paste(atlas_network, metric, "file loaded"))
...
make_EdgeDemog <- function(atlas_network, demog_csv, subj_id, dataset){
  subxedge.matrix <- readRDS(sprintf("/cbica/projects/network_replication/output/%1$s/sensitivity_analyses/edge/subxedge_%2$s.RData", dataset, atlas_network))
  print("file loaded")
  ...
  saveRDS(edgeDemog_df, sprintf("/cbica/projects/network_replication/output/%1$s/sensitivity_analyses/edge/%2$s_demographics_finalsample.RData", dataset, atlas_network))
...
dev_derivatives <- function(parcel.labels, metric, atlas_name, dataset_name){
  ...
  write.csv(gam.derivatives.atlas, sprintf("/cbica/projects/network_replication/output/%1$s/sensitivity_analyses/%2$s/GAM/%3$s.csv", dataset_name, metric, filename), row.names = F, quote = F)
...
dev_derivatives_covbat <- function(parcel.labels, metric, atlas_name, dataset_name){
  ...
  gam.derivatives.atlas <- left_join(gam.derivatives.atlas, SAaxis, by="label", sort = F)
  filename <- paste0("gam.", metric, ".derivatives.", atlas_name)
  write.csv(gam.derivatives.atlas, sprintf("/cbica/projects/network_replication/output/%1$s/sensitivity_analyses/%2$s/GAM/%3$s_covbat.csv", dataset_name, metric, filename), row.names = F, quote = F)
  return(gam.derivatives.atlas)
}
...
make_deriv.SAaxis.posteriorcor <- function(metric, atlas_name, dataset_name){
  ...
  write.table(deriv.SAaxis.posteriorcor, sprintf("/cbica/projects/network_replication/output/%1$s/sensitivity_analyses/%2$s/GAM/SAaxis_posteriorderivative_correlation_byage_%3$s.csv", dataset_name, metric, atlas_name), quote = FALSE, col.names = FALSE, row.names = FALSE, sep = ",")
