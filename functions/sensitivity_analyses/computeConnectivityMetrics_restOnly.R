...
computeGBC_restOnly <- function(subject, atlas, dataset){
  ...
  connect.matrix <- readRDS(sprintf("/cbica/projects/network_replication/input/%1$s/connMatricesData/connectivity_matrices_restOnly/%2$s_ConnMatrices.RData",dataset, subject))
...
# @param dataset A character string, name of dataset
computeBNC_WNC_restOnly <- function(subject, atlas, metric, dataset){
...
extractParcel2ParcelConn <- function(subject, atlas, dataset){
  ...
  if(dataset == "PNC") {
    connect.matrix <- readRDS(sprintf("/cbica/projects/network_replication/input/PNC/connMatricesData/connectivity_matrices_restOnly/%1$s_ConnMatrices.RData",subject))
    ...
    connect.matrix <- readRDS(sprintf("/cbica/projects/network_replication/input/HCPD/connMatricesData/connectivity_matrices_restOnly/%1$s_ConnMatrices.RData", subject))
    if(str_detect(atlas, "schaefer200") | str_detect(atlas, "schaefer400")) {
      atlas_name <- paste0(str_extract(atlas, "schaefer[0-9]"), "17")
    } else {
      atlas_name <- atlas
