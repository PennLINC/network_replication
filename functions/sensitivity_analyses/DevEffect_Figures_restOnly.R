...
make_gam.smooths_centered <- function(metric, atlas, dataset){
  ...
  if (dataset=="PNC"){ #fixing formatting issue with sensitivity analysis output
    demog <- select(demog, -sub)
  } else {
    demog <- demog
  }
  if (dataset=="NKI"){
  ...
  if (dataset=="HCPD") {
    end_parcels <- start_parcels + length(get(paste0(atlas, ".parcel.labels"))) - 1
  } else {
...
make_repParcel.fig <- function(metric, atlas, parcels, color_hexcode) {
  ...
  main.plot <- ggplot(smooth_fits,aes(age,est,group=SA.axis_rank)) + 
    geom_line(data = smooth_fits, size=1.5, alpha = .6,  colour=color_hexcode) + ylim(-0.035, 0.047) + xlim(min(smooth_fits$age), max(smooth_fits$age)) + 
    ggtitle(paste(metric, "-", atlas, parcels)) + ylab("Predicted Global Brain Connectivity") + 
    theme_classic()  
...
make_surface_plot <- function(atlas) {
  ...
  surface.plot.fig <- gg_tensor(g2)+theme_classic(base_size = 20) +
    ...
    scale_fill_gradient2(  
                         ...
                         guide = guide_colorbar(legend.position='top',
                                                title.position="top",
                                                ticks.colour = "gray50",
                                                ticks.linewidth = 2)) +
    ggtitle(atlas) + 
