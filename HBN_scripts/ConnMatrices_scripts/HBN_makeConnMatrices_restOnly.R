...
HBN_CIFTI_summary  <- readRDS("/cbica/projects/network_replication/input/HBN/sample_selection/HBN_FinalSample_withCUBIDS_restOnly_20230223.RData") 
participants <- unique(HBN_CIFTI_summary[[1]]$sub)
#demographics <- read.csv("/cbica/projects/network_replication/input/HBN/HBN_demographics.csv")
...
listCifti <- function(subject, list_filepath, ciftiFiles_df){
  ...
  saveRDS(cifti_list, paste0("/cbica/projects/network_replication/input/HBN/connMatricesData/cifti_lists_restOnly/", subject, "_timeseries_restOnly.RData"))
  ...
  saveRDS(matrix_output, paste0("/cbica/projects/network_replication/input/HBN/connMatricesData/connectivity_matrices_restOnly/", subject,"_ConnMatrices.RData"))
  rm(cifti_list)
  rm(matrix_output)
  gc()
}
