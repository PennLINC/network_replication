...
# remove participants missing age data in HBN (which may be included in the subxparcel output - make sure to double check!!)
...
for(i in c(1:length(atlases_GBC))){
  ...
  write.csv(get(df_name), sprintf("/cbica/projects/network_replication/output/%1$s/%2$s/%2$s_subxparcel_matrix_%3$s.csv", dataset, metric, atlases_GBC[i]))
  print(df_name)
...
for(i in c(1:length(atlases))){
  ...
  assign(df_name, get(df_name)[-participants_noAge_index,])
  write.csv(get(df_name), sprintf("/cbica/projects/network_replication/output/%1$s/%2$s/%2$s_subxparcel_matrix_%3$s.csv", dataset, metric, atlases[i]))
  print(df_name)
  print(dim(get(df_name)))
} 
