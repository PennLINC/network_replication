...
subxparcel.matrix <- read.csv("/Users/audluo/cbica/projects/network_replication/revisions/output/NKI/GBC/GBC_subxparcel_matrix_schaefer200x7_orig.csv")
GBC.schaefer200_NKI <- merge(subxparcel.matrix, demographics, by="subject")
   
GBC.schaefer200_NKI$sex <- as.factor(GBC.schaefer200_NKI$sex) 
```
...
for(row in c(1:length(parcel.labels))){ #for each region
  ...
  GAM.RESULTS <- gam.fit(measure = "GBC", atlas = "schaefer200", conn_type = "NKI", region = region, smooth_var = "age", covariates = "sex + meanFD_avgSes") #run the gam.fit function
...
write.csv(gam.age, sprintf("/cbica/projects/network_replication/revisions/output/%1$s/%2$s/GAM/GAMresults.%2$s.age.%3$s%4$s_%5$s.csv", "NKI", "GBC", "schaefer200", "x7", "orig"), row.names = F, quote = F)
