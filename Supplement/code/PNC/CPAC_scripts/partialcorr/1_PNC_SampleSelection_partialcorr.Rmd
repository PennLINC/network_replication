---
title: "PNC Sample Selection - for CPAC partial correlation"
author: "Audrey Luo"
date: "2023-11-30"
...
1) original sample: N=1537, ages 8-23 - N=1505 after filtering for n-back
2) exclude participants with medical conditions affecting brain function and gross neurological abnormalities: N=1365,  
3) include passing T1 QC: N=1332 
4) include meanFD < 0.3: N=1127 
5) include scans with at least 7 minutes of scan time: N= 1114, 588 females.
 

 
range after: 7.05 to 11.55 min
median after: 11.555 min

Age: mean = 15.6, SD = 3.4
...
```{r}
qc <- read.table('/cbica/projects/network_replication/revisions/input/PNC/CPAC_subfiles/sub-2726043986/sub-2726043986_ses-PNC1_task-frac2back_acq-VARIANTNoFmap_desc-powerParams_motion.txt', sep=',', header=T)
qc
...
collated_PNC.CPAC <- collated_PNC.CPAC %>% dplyr::filter(str_detect(Scan, "frac2back")) %>% dplyr::rename(sub=Subject, meanFD=MeanFD_Power) # keep only n-back
length(unique(collated_PNC.CPAC$sub)) # 1505 participants with n-back
...
nrow(demographics) # 1505 total with demographics and n-back
...
length(unique(collated_PNC.CPAC_medExclusion$sub)) # N=1365
...
length(unique(collated_PNC.CPAC_T1Exclusion$sub)) # N=1332 after excluding participants with failed T1
 
collated_PNC.CPAC_headMotion <- collated_PNC.CPAC_T1Exclusion[c(which(collated_PNC.CPAC_T1Exclusion$meanFD < 0.3)),]
length(unique(collated_PNC.CPAC_headMotion$sub)) # N=1127 after excluding meanFD >= 0.3  
 
 
write.csv(collated_PNC.CPAC_headMotion, "/cbica/projects/network_replication/revisions/input/PNC/CPAC_sample_selection/CPAC_partialcorr_collated_PNC_headMotion.csv", row.names=F)
```
...
collated_PNC.CPAC_headMotion <- read.csv("/cbica/projects/network_replication/revisions/input/PNC/CPAC_sample_selection/CPAC_partialcorr_collated_PNC_headMotion.csv")
...
collated_PNC.CPAC_headMotion <- collated_PNC.CPAC_headMotion %>%  mutate(task=str_extract(Scan, "frac2back")) %>%
...
range(fMRIinclude_CUBIDS_ScanTime$ScanTime_Total) # 3.60 to 11.55
median(fMRIinclude_CUBIDS_ScanTime$ScanTime_Total) # 11.55 min
getmode <- function(v) {
   uniqv <- unique(v)
   uniqv[which.max(tabulate(match(v, uniqv)))]
}
getmode(fMRIinclude_CUBIDS_ScanTime$ScanTime_Total) #11.55 min
...
length(unique(fMRIinclude_CUBIDS$sub)) # N=1114
 


fMRIinclude_CUBIDS_ScanTime <- fMRIinclude_CUBIDS %>% group_by(sub) %>% summarise(ScanTime_Total = sum(ScanTimeMinutes))
 
max(fMRIinclude_CUBIDS_ScanTime$ScanTime_Total) # 11.55 minutes 
range(fMRIinclude_CUBIDS_ScanTime$ScanTime_Total) # 7.05 11.55
median(fMRIinclude_CUBIDS_ScanTime$ScanTime_Total) # 11.55

fMRIinclude_CUBIDS_NumVols <- fMRIinclude_CUBIDS %>% group_by(sub) %>% summarise(NumVols_Total = sum(NumVolumes))
max(fMRIinclude_CUBIDS_NumVols$NumVols_Total) # 231 volumes

saveRDS(fMRIinclude_CUBIDS, "/cbica/projects/network_replication/revisions/input/PNC/CPAC_sample_selection/CPAC_partialcorr_fMRIinclude_CUBIDS_20231130.RData")
...
fMRIinclude_CUBIDS <- readRDS("/cbica/projects/network_replication/revisions/input/PNC/CPAC_sample_selection/CPAC_partialcorr_fMRIinclude_CUBIDS_20231130.RData")
...
length(which(final_dem_df$sex=="2")) # 588 females 
...
mean(final_dem_df$age) # mean = 15.6
sd(final_dem_df$age) # SD = 3.4

write.csv(final_dem_df, "/cbica/projects/network_replication/revisions/input/PNC/CPAC_sample_selection/CPAC_partialcorr_PNC_demographics_finalsample_20231130.csv", row.names=F)
