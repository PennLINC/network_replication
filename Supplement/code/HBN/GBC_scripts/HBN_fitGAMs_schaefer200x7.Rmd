...
subxparcel.matrix <- read.csv("/cbica/projects/network_replication/revisions/output/HBN/GBC/GBC_subxparcel_matrix_schaefer200x7_orig.csv")
GBC.schaefer200_HBN <- merge(subxparcel.matrix, demographics, by="subject")
   
GBC.schaefer200_HBN$sex <- as.factor(GBC.schaefer200_HBN$sex) 
```
...
for(row in c(1:length(parcel.labels))){ #for each region
  ...
  GAM.RESULTS <- gam.fit(measure = "GBC", atlas = "schaefer200", conn_type = "HBN", region = region, smooth_var = "age", covariates = "sex + meanFD_avgSes") #run the gam.fit function
...
write.csv(gam.age, sprintf("/cbica/projects/network_replication/revisions/output/%1$s/%2$s/GAM/GAMresults.%2$s.age.%3$s%4$s_%5$s.csv", "HBN", "GBC", "schaefer200", "x7", "orig"), row.names = F, quote = F)
```
