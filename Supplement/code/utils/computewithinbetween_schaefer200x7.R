...
outdir <- sprintf("/cbica/projects/network_replication/revisions/output/%1$s/withinbetween/", dataset)
...
computeMeanNetworkConn <- function(subject, atlas) {
  ...
  # for parcels in each network, find the connectivity (correlation) with all parcels for each of the other networks
  # then take the average of the initial network with the other networks
...
saveRDS(subjects_mean_netconn, sprintf("%1$swithinbetween_subxnetpair_matrix_schaefer200x7_orig_notbound.RData", outdir))
...
write.csv(subjects_mean_netconn, sprintf("%1$swithinbetween_subxnetpair_matrix_schaefer200x7_orig.csv", outdir), row.names=F, quote=F)
