---
title: "PNC Figures - Sensitivity Analysis"
author: "Audrey Luo"
...
gam.GBC.age.schaefer200 <- read.csv(sprintf("/cbica/projects/network_replication/output/%1$s/sensitivity_analyses/%2$s/GAM/GAMresults.%2$s.age.%3$s.csv", dataset, metric, atlas))
...
write.csv(GBC.axis_schaefer200, sprintf("/cbica/projects/network_replication/output/%1$s/sensitivity_analyses/%2$s/GAM/%3$s.csv", dataset, metric, "GBC.axis_schaefer200"))
```

#### load figure df's - metric.axis_atlas
```{r load_figureDfs, include=FALSE, cache=TRUE}
#loads GBC.axis_schaefer200 
GBC.axis_schaefer200 <- read.csv(sprintf("/cbica/projects/network_replication/output/%1$s/sensitivity_analyses/%2$s/GAM/%3$s.csv", dataset, metric, "GBC.axis_schaefer200"))
```
...
write.csv(spearman_rho, "/cbica/projects/network_replication/output/PNC/sensitivity_analyses/spearman_rho.csv")
...
saveRDS(p_spin_schaefer200, "/cbica/projects/network_replication/output/PNC/sensitivity_analyses/pspin_vals.RData")
...
spearman_rho <- read.csv("/cbica/projects/network_replication/output/PNC/sensitivity_analyses/spearman_rho.csv")
pspin_vals <- readRDS("/cbica/projects/network_replication/output/PNC/sensitivity_analyses/pspin_vals.RData")
...
ggsave("/Users/audluo/Library/CloudStorage/Box-Box/Box_PhD_Land/PennLINC/network_replication/Writing/Manuscript/Figures/Figures_Illustrator/Supplementary/FigS3_GBCAgeEffect_PNC.pdf", dpi=300, width = 5, height = 5)
...
gam.smooths.schaefer200.GBC <- make_gam.smooths(metric, atlas, dataset)
```

#### make devTraj df
- makes df for trajectories. Can load the output in following chunk.
```{r make_devTraj_centered, include=FALSE}
devTraj.centered.schaefer200.GBC <-  make_gam.smooths_centered(metric, atlas, dataset)
```
#### load devTraj df
```{r load_devTraj_centered.dfs, include=FALSE, cache=TRUE}
devTraj.centered.schaefer200.GBC_df <- read.csv(sprintf("/cbica/projects/network_replication/output/%1$s/sensitivity_analyses/%2$s/GAM/GAMsmoothfits.%2$s.age.%3$s_centered.csv", dataset, metric, atlas))
```
...
ggsave("/Users/audluo/Library/CloudStorage/Box-Box/Box_PhD_Land/PennLINC/network_replication/Writing/Manuscript/Figures/Figures_Illustrator/Supplementary/FigS3_GBCDevTraj_PNC.pdf", dpi=300, width = 5, height = 5)
