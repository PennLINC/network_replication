...
for(i in c(1:length(atlases_GBC))){
  ...
  assign(df_name, read.csv(sprintf("/cbica/projects/network_replication/output/%1$s/sensitivity_analyses/%2$s/GAM/GAMresults.%2$s.age.%3$s.csv", dataset, metric, atlases_GBC[i])))
...
for(i in c(1:length(atlases))){
  ...
  assign(df_name, read.csv(sprintf("/cbica/projects/network_replication/output/%1$s/sensitivity_analyses/%2$s/GAM/GAMresults.%2$s.age.%3$s.csv", dataset, metric, atlases[i])))
  assign(df_name, get(df_name) %>% select(-label))
...
for(i in c(1:length(atlases_GBC))){
  ...
  #write.csv(get(filename), sprintf("/cbica/projects/network_replication/output/%1$s/sensitivity_analyses/%2$s/GAM/%3$s.csv", dataset, metric, filename))
  print(df_name)
} 
...
WNC.axis_schaefer200x7_1 <- WNC.axis_schaefer200x7
WNC.axis_schaefer200x7 <- WNC.axis_schaefer200x7[c(which(WNC.axis_schaefer200x7$SA.axis_rank > 170 & WNC.axis_schaefer200x7$SA.axis_rank < 201)),]
ageEffect_surf_figure('schaefer200x7', "WNC")
#WNC.axis_schaefer200x7$GAM.age.AdjRsq[c(which(WNC.axis_schaefer200x7$GAM.age.AdjRsq < 0))]
  
cbind(WNC.axis_schaefer200x7$label[c(which(WNC.axis_schaefer200x7$GAM.age.AdjRsq > 0))], round(WNC.axis_schaefer200x7$GAM.age.AdjRsq[c(which(WNC.axis_schaefer200x7$GAM.age.AdjRsq > 0))], 4), round(WNC.axis_schaefer200x7$Anova.age.pvalue.fdr[c(which(WNC.axis_schaefer200x7$GAM.age.AdjRsq > 0))], 4))
```
...
ageEffect_GBC.fig$schaefer200
#ggsave("/Users/audluo/Library/CloudStorage/Box-Box/Box_PhD_Land/PennLINC/network_replication/Figures/OHBM_2023/GBC_schaefer200_SurfaceView_PNC.svg", dpi=300, width = 5, height = 4)
...
#saveRDS(list(p_spin_GBC, p_spin_BNC, p_spin_WNC), "/cbica/projects/network_replication/output/PNC/pspin_vals.RData")
```
...
r_text_BNC.schaefer200x7 <- expression(paste(italic("r"), "= -0.59, ",  , italic(p[spin]), "< 0.0001"))
(BNC_schaefer200x7 <- gam_figure_p.spin(BNC.axis_schaefer200x7, "BNC", "schaefer200x7", r_text_BNC.schaefer200x7, x_pos= 160, y_pos=0.1))

r_text_BNC.schaefer200x17 <- expression(paste(italic("r"), "= -0.69, ",  , italic(p[spin]), "< 0.0001"))
...
r_text_BNC.schaefer400x7 <- expression(paste(italic("r"), "= -0.71, ",  , italic(p[spin]), "< 0.0001"))
...
r_text_WNC.schaefer200x7 <- expression(paste(italic("r"), "= -0.36, ",  , italic(p[spin]), "< 0.01"))
(WNC_schaefer200x7 <- gam_figure_p.spin(WNC.axis_schaefer200x7, "WNC", "schaefer200x7", r_text_WNC.schaefer200x7, x_pos= 160, y_pos=0.1))

r_text_WNC.schaefer200x17 <- expression(paste(italic("r"), "= -0.44, ",  , italic(p[spin]), "< 0.05"))
...
r_text_WNC.schaefer400x7 <- expression(paste(italic("r"), "= -0.43, ",  , italic(p[spin]), "< 0.0001"))
...
```{r Fig1, fig.height= 8, fig.width= 16, warning=FALSE, cache=TRUE}
...
#### Correlation between Age Effect of BNC vs. WNC
...
for(i in c(1:length(atlases))){
  ...
  assign(df_name, make_gam.smooths(metric, atlases[i], dataset))
  print(df_name)
} 
...
for(i in c(1:length(atlases))){
  ...
  #assign(df_name, make_gam.smooths_centered(metric, atlases[i], dataset)) #commenting out to prevent make_gam.smooths_centered from making new csv's each run
  print(df_name)
} 
...
for(i in c(1:length(atlases))){
  df_name <- paste0("devTraj.centered.", atlases[i], ".", metric, "_df")
  assign(df_name, read.csv(sprintf("/cbica/projects/network_replication/output/%1$s/sensitivity_analyses/%2$s/GAM/GAMsmoothfits.%2$s.age.%3$s_centered.csv", dataset, metric, atlases[i])))
  print(df_name)
}
...
#save.image(file='PNC_devEffectFigures_environment.RData')
```
...
schaefer200x7_GBC_SomMot <- make_repParcel.fig("GBC", "schaefer200x7", "SomMot", "#24A6A8FF", SA_rank=40, "schaefer7_200", c(0.06, 0.06, 0.06, 0.07)) 
schaefer200x7_BNC_SomMot <- make_repParcel.fig("BNC", "schaefer200x7",  "SomMot", "#24A6A8FF", SA_rank=40, "schaefer7_200", c(0.05, 0.027, 0.01, 0.07))
schaefer200x7_WNC_SomMot <- make_repParcel.fig("WNC", "schaefer200x7",  "SomMot", "#24A6A8FF", SA_rank=40, "schaefer7_200", c(0.17, 0.13, 0.14, 0.20)) 
...
schaefer200x7_GBC_default <- make_repParcel.fig("GBC", "schaefer200x7", "default", "#C73000FF", SA_rank=155, "schaefer7_200", c(0.052, 0.032, 0.02, 0.072)) 
schaefer200x7_BNC_default <- make_repParcel.fig("BNC", "schaefer200x7",  "default", "#C73000FF", SA_rank=155, "schaefer7_200", c(0.052, 0.032, 0.02, 0.072))
schaefer200x7_WNC_default <- make_repParcel.fig("WNC", "schaefer200x7",  "default", "#C73000FF", SA_rank=155, "schaefer7_200", c(0.2, 0.14, 0.16, 0.22)) 
...
schaefer200x7_GBC_frontoparietalControl <- make_repParcel.fig("GBC", "schaefer200x7", "frontoparietalControl", "#C73000FF", SA_rank=143, "schaefer7_200", c(0.052, 0.025, 0.01, 0.07)) 
schaefer200x7_BNC_frontoparietalControl <- make_repParcel.fig("BNC", "schaefer200x7",  "frontoparietalControl", "#C73000FF", SA_rank=143, "schaefer7_200", c(0.052, 0.025, 0.01, 0.07))
schaefer200x7_WNC_frontoparietalControl <- make_repParcel.fig("WNC", "schaefer200x7",  "frontoparietalControl", "#C73000FF", SA_rank=143, "schaefer7_200", c(0.1, 0.06, 0.06, 0.13)) 
...
schaefer200x7_GBC_attention <- make_repParcel.fig("GBC", "schaefer200x7", "attention", "#EEAC32FF", SA_rank=113, "schaefer7_200", c(0.052, 0.027, 0.02, 0.072)) 
schaefer200x7_BNC_attention <- make_repParcel.fig("BNC", "schaefer200x7",  "attention", "#EEAC32FF", SA_rank=113, "schaefer7_200", c(0.052, 0.027, 0.02, 0.072))
schaefer200x7_WNC_attention <- make_repParcel.fig("WNC", "schaefer200x7",  "attention", "#EEAC32FF", SA_rank=113, "schaefer7_200", c(0.2, 0.14, 0.16, 0.22)) 
...
```{r}
#plot.with.inset_frontoparietalControl
```



```{r fig.height=3, fig.width=3}
schaefer200x7_GBC_attention <- make_repParcel.fig("GBC", "schaefer200x7", "attention", "#EEAC32FF", SA_rank=113, "schaefer7_200", c(0.03, 0.023, 0.024, 0.038)) 
schaefer200x7_BNC_attention <- make_repParcel.fig("BNC", "schaefer200x7",  "attention", "#EEAC32FF", SA_rank=113, "schaefer7_200", c(0.03, 0.023, 0.024, 0.038))
schaefer200x7_WNC_attention <- make_repParcel.fig("WNC", "schaefer200x7",  "attention", "#EEAC32FF", SA_rank=113, "schaefer7_200", c(0.12, 0.07, 0.08, 0.1)) 
schaefer200x7_attention <- list(schaefer200x7_GBC_attention, schaefer200x7_BNC_attention, schaefer200x7_WNC_attention)
names(schaefer200x7_attention) <- c("GBC", "BNC", "WNC")


 
schaefer200x7_attention$WNC
...
####Schaefer200x17 representative parcels
...
names(schaefer200x17_SAaxis)[3] <- "label"
# schaefer200x17
## extract row numbers for parcels of interest (somatomotor, default, visual, salient/ventral attention, and frontal parietal networks)
## note that all 7 Network schaefer atlas parcels have been reordered to match 17 network (see schaefer17to17_reordering.Rmd)
## arrange(schaefer200x17_SAaxis, by= SA.axis_rank) 
SM <- which(str_detect(schaefer200x17_SAaxis$label, "SomMot")) # mean SA rank = 40
schaefer200x17_SAaxis$label[c(which(str_detect(schaefer200x17_SAaxis$label, "SomMot")))]
SM_ranks <- schaefer200x17_SAaxis$SA.axis_rank[SM] # somatomotor is pretty uniform in terms of all increasing; auditory too though there are some U shaped curves
mean(SM_ranks)
visual <- which(str_detect(schaefer200x17_SAaxis$label, "Vis")) # mean SA rank = 25
visual_ranks <- schaefer200x17_SAaxis$SA.axis_rank[visual]
mean(visual_ranks)
default <- which(str_detect(schaefer200x17_SAaxis$label, "Default")) # mean SA rank = 155
default_ranks <- schaefer200x17_SAaxis$SA.axis_rank[default]  
mean(default_ranks)
attention <- which(str_detect(schaefer200x17_SAaxis$label, "SalVentAttnA")) # mean SA rank = 123
attention_ranks <- schaefer200x17_SAaxis$SA.axis_rank[attention]  
mean(attention_ranks)
schaefer200x17_SAaxis$region[attention]
frontoparietalControl <- which(str_detect(schaefer200x17_SAaxis$label, "Cont")) # mean SA rank = 144
frontoparietalControl_ranks <- schaefer200x17_SAaxis$SA.axis_rank[frontoparietalControl]  
 
schaefer200x17_parcelRanks <- list(SM_ranks, visual_ranks, default_ranks, attention_ranks, frontoparietalControl_ranks)
repParcels_GBC_schaefer200x17 <- lapply(schaefer200x17_parcelRanks, smooths_repParcels, "schaefer200", "GBC")  
names(repParcels_GBC_schaefer200x17) <- c("SomMot", "visual", "default", "attention", "frontoparietalControl")
repParcels_BNC_schaefer200x17 <- lapply(schaefer200x17_parcelRanks, smooths_repParcels, "schaefer200x17", "BNC")  
names(repParcels_BNC_schaefer200x17) <- c("SomMot", "visual", "default", "attention", "frontoparietalControl")
repParcels_WNC_schaefer200x17 <- lapply(schaefer200x17_parcelRanks, smooths_repParcels, "schaefer200x17", "WNC")  
names(repParcels_WNC_schaefer200x17) <- c("SomMot", "visual", "default", "attention", "frontoparietalControl")
repParcels_schaefer200x17 <- list(repParcels_GBC_schaefer200x17, repParcels_BNC_schaefer200x17, repParcels_WNC_schaefer200x17)
names(repParcels_schaefer200x17) <- c("GBC", "BNC", "WNC")
 

```
 

```{r FigRepresentativeParcels_schaefer200x17, fig.height= 8, fig.width= 12, warning=FALSE, cache=TRUE}

## set up dataframe for making figures
schaefer200x17_SAaxis$SA.axis_rank <- as.numeric(schaefer200x17_SAaxis$SA.axis_rank)
names(schaefer200x17_SAaxis)[3] <- "region"
schaefer200x17_SAaxis_fig.df <- schaefer200x17_SAaxis %>% mutate(SomMot = ifelse(SA.axis_rank %in% SM_ranks, 1, 0)) %>% 
  mutate(default = ifelse(SA.axis_rank %in% default_ranks, 1, 0))  %>%  
  mutate(attention = ifelse(SA.axis_rank %in% attention_ranks, 1, 0)) %>% 
  mutate(visual = ifelse(SA.axis_rank %in% visual_ranks, 1, 0)) %>% 
  mutate(frontoparietalControl = ifelse(SA.axis_rank %in% frontoparietalControl_ranks, 1, 0))
schaefer200x17_SAaxis_fig.df$region <- gsub("Network", "7Network", schaefer200x17_SAaxis_fig.df$region)

 
# schaefer200x17  
## somatomotor parcels: average SA rank = 39.5 
schaefer200x17_GBC_SomMot <- make_repParcel.fig("GBC", "schaefer200x17", "SomMot", "#24A6A8FF", SA_rank=40, "schaefer17_200", c(0.06, 0.06, 0.06, 0.07)) 
schaefer200x17_BNC_SomMot <- make_repParcel.fig("BNC", "schaefer200x17",  "SomMot", "#24A6A8FF", SA_rank=40, "schaefer17_200", c(0.05, 0.027, 0.01, 0.07))
schaefer200x17_WNC_SomMot <- make_repParcel.fig("WNC", "schaefer200x17",  "SomMot", "#24A6A8FF", SA_rank=40, "schaefer17_200", c(0.17, 0.13, 0.14, 0.20)) 
schaefer200x17_SomMot <- list(schaefer200x17_GBC_SomMot, schaefer200x17_BNC_SomMot, schaefer200x17_WNC_SomMot)
names(schaefer200x17_SomMot) <- c("GBC", "BNC", "WNC")

## default parcels: average SA rank = 155
schaefer200x17_GBC_default <- make_repParcel.fig("GBC", "schaefer200x17", "default", "#C73000FF", SA_rank=155, "schaefer17_200", c(0.052, 0.032, 0.02, 0.072)) 
schaefer200x17_BNC_default <- make_repParcel.fig("BNC", "schaefer200x17",  "default", "#C73000FF", SA_rank=155, "schaefer17_200", c(0.052, 0.032, 0.02, 0.072))
schaefer200x17_WNC_default <- make_repParcel.fig("WNC", "schaefer200x17",  "default", "#C73000FF", SA_rank=155, "schaefer17_200", c(0.2, 0.14, 0.16, 0.22)) 
schaefer200x17_default <- list(schaefer200x17_GBC_default, schaefer200x17_BNC_default, schaefer200x17_WNC_default)
names(schaefer200x17_default) <- c("GBC", "BNC", "WNC")

## frontoparietalControl network parcels: average SA rank = 143  
schaefer200x17_GBC_frontoparietalControl <- make_repParcel.fig("GBC", "schaefer200x17", "frontoparietalControl", "#C73000FF", SA_rank=143, "schaefer17_200", c(0.052, 0.025, 0.01, 0.07)) 
schaefer200x17_BNC_frontoparietalControl <- make_repParcel.fig("BNC", "schaefer200x17",  "frontoparietalControl", "#C73000FF", SA_rank=143, "schaefer17_200", c(0.052, 0.025, 0.01, 0.07))
schaefer200x17_WNC_frontoparietalControl <- make_repParcel.fig("WNC", "schaefer200x17",  "frontoparietalControl", "#C73000FF", SA_rank=143, "schaefer17_200", c(0.1, 0.06, 0.06, 0.13)) 
schaefer200x17_frontoparietalControl <- list(schaefer200x17_GBC_frontoparietalControl, schaefer200x17_BNC_frontoparietalControl, schaefer200x17_WNC_frontoparietalControl)  
names(schaefer200x17_frontoparietalControl) <- c("GBC", "BNC", "WNC")

## attention network parcels: average SA rank = 113  

schaefer200x17_GBC_attention <- make_repParcel.fig("GBC", "schaefer200x17", "attention", "#EEAC32FF", SA_rank=113, "schaefer17_200", c(0.052, 0.027, 0.02, 0.072)) 
schaefer200x17_BNC_attention <- make_repParcel.fig("BNC", "schaefer200x17",  "attention", "#EEAC32FF", SA_rank=113, "schaefer17_200", c(0.052, 0.027, 0.02, 0.072))
schaefer200x17_WNC_attention <- make_repParcel.fig("WNC", "schaefer200x17",  "attention", "#EEAC32FF", SA_rank=113, "schaefer17_200", c(0.2, 0.14, 0.16, 0.22)) 
schaefer200x17_attention <- list(schaefer200x17_GBC_attention, schaefer200x17_BNC_attention, schaefer200x17_WNC_attention)
names(schaefer200x17_attention) <- c("GBC", "BNC", "WNC")

main.plot_attention <- ggplot(smooth_fits_attention,aes(age,est,group=SA.axis_rank)) + 
      geom_line(data = smooth_fits_attention, size=1.5, alpha = .6,  colour="#EEAC32FF") + 
      ggtitle(paste(metric, "-", "Schaefer200", "attention")) + ylim(min(smooth_fits_attention$est), 0.04) + annotate(geom="text", x=20, y=0.027, label="Mean S-A Rank = 113", color="black", fontface="bold") + 
      theme_classic()
attention_schaefer200.fig_inset <- ggplot() + geom_brain(data=schaefer200x17_SAaxis_fig.df, 
                      atlas=schaefer17_200, 
                      hemi='left', side = 'lateral',
                      mapping=aes(fill=attention, colour=I("#EEAC32FF"),  size=I(0.5)), 
                      show.legend=FALSE) + scale_fill_gradient(low="white", high = "#EEAC32FF") + theme_void()
plot.with.inset_attention <- main.plot_attention + 
  annotation_custom(ggplotGrob(attention_schaefer200.fig_inset), 
    xmin = 18, xmax = 22, ymin = 0.02, ymax = 0.057)

 
## visual network parcels: average SA rank = 30

#plot_grid(plot.with.inset_SM, plot.with.inset_attention, plot.with.inset_default, plot.with.inset_frontoparietalControl)
...
```{r}
schaefer200x17_GBC_attention <- make_repParcel.fig("GBC", "schaefer200x17", "attention", "#EEAC32FF", SA_rank=123, "schaefer17_200", c(0.03, 0.023, 0.024, 0.038)) 
schaefer200x17_BNC_attention <- make_repParcel.fig("BNC", "schaefer200x17",  "attention", "#EEAC32FF", SA_rank=123, "schaefer17_200", c(0.03, 0.023, 0.024, 0.038))
schaefer200x17_WNC_attention <- make_repParcel.fig("WNC", "schaefer200x17",  "attention", "#EEAC32FF", SA_rank=123, "schaefer17_200", c(0.12, 0.07, 0.08, 0.1)) 
schaefer200x17_attention <- list(schaefer200x17_GBC_attention, schaefer200x17_BNC_attention, schaefer200x17_WNC_attention)
names(schaefer200x17_attention) <- c("GBC", "BNC", "WNC")


 
schaefer200x17_attention$GBC
#ggsave("/Users/audluo/Library/CloudStorage/Box-Box/Box_PhD_Land/PennLINC/network_replication/Figures/OHBM_2023/GBC_schaefer200_attention.svg", dpi=300, width = 4, height = 4)


 
schaefer200x17_SomMot$GBC
#ggsave("/Users/audluo/Library/CloudStorage/Box-Box/Box_PhD_Land/PennLINC/network_replication/Figures/OHBM_2023/GBC_schaefer200_SomMot.svg", dpi=300, width = 4, height = 4)


schaefer200x17_default$GBC
#ggsave("/Users/audluo/Library/CloudStorage/Box-Box/Box_PhD_Land/PennLINC/network_replication/Figures/OHBM_2023/GBC_schaefer200_default.svg", dpi=300, width = 4, height = 4)
...
for(i in c(1:length(atlases_GBC))){
  ...
  assign(df_name, read.csv(sprintf("/cbica/projects/network_replication/output/%1$s/sensitivity_analyses/%2$s/GAM/%3$s.csv", dataset, metric, df_name)))
  print(df_name)
} 
...
For each parcel, we take 1000 derivatives at 200 ages (and do this 10 times for total of 10k draws). So for each age, there are 1000 draws (200 ages * 1000 draws per age  * 200 parcels = 40 million total draws across the brain for schaefer200)

Then take the correlation between parcel rank and derivative at each age for each draw. so every draw has 200 ages, and at each of those 200 ages, we have an SAcorrelation. The SAcorrelation is computed  by taking the correlation between parcel rank and parcel posterior derivative at a given draw and age. So for draw 1 of age 8, take the SAcorrelation for all the parcels (a correlation of 200 parcels). Then for draw 1 of age 8.067, again take the correlation of all the parcels. Etc. 
Then produce a data frame of SA correlations across all parcels for a given draw and age.

So the median (black line) is the median correlation across all draws at a specific age. But that correlation is done across all parcels. 
...
load_posterior_deriv <- function(atlas_name, metric){
  ...
  assign(df_name, readRDS(sprintf("/cbica/projects/network_replication/output/%1$s/sensitivity_analyses/%2$s/GAM/SAaxis_posteriorderivative_correlation_byage_%3$s.RData", dataset, metric, atlas_name)))
...
saveRDS(deriv.SAaxis.posteriorcorrs_GBC, "/cbica/projects/network_replication/output/PNC/GBC/GAM/deriv.SAaxis.posteriorcorrs_GBC.RData")

deriv.SAaxis.posteriorcorrs_BNC <- lapply(atlases, load_posterior_deriv, "BNC")
names(deriv.SAaxis.posteriorcorrs_BNC) <- atlases 
saveRDS(deriv.SAaxis.posteriorcorrs_BNC, "/cbica/projects/network_replication/output/PNC/BNC/GAM/deriv.SAaxis.posteriorcorrs_BNC.RData")

deriv.SAaxis.posteriorcorrs_WNC <- lapply(atlases, load_posterior_deriv, "WNC")
names(deriv.SAaxis.posteriorcorrs_WNC) <- atlases 
saveRDS(deriv.SAaxis.posteriorcorrs_WNC, "/cbica/projects/network_replication/output/PNC/WNC/GAM/deriv.SAaxis.posteriorcorrs_WNC.RData")
...
deriv.SAaxis.posteriorcorrs_GBC <- readRDS("/cbica/projects/network_replication/output/PNC/GBC/GAM/deriv.SAaxis.posteriorcorrs_GBC.RData") 

deriv.SAaxis.posteriorcorrs_BNC <- readRDS("/cbica/projects/network_replication/output/PNC/BNC/GAM/deriv.SAaxis.posteriorcorrs_BNC.RData")  

deriv.SAaxis.posteriorcorrs_WNC <- readRDS("/cbica/projects/network_replication/output/PNC/WNC/GAM/deriv.SAaxis.posteriorcorrs_WNC.RData" ) 
...
#saveRDS(age.max.stats_GBC, "/cbica/projects/network_replication/output/PNC/GBC/GAM/age.max.stats_GBC.RData")

age.max.stats_BNC <- lapply(atlases, function_age.max.corr, "BNC")
names(age.max.stats_BNC) <- atlases 
#saveRDS(age.max.stats_BNC, "/cbica/projects/network_replication/output/PNC/BNC/GAM/age.max.stats_BNC.RData")

age.max.stats_WNC <- lapply(atlases, function_age.max.corr, "WNC")
names(age.max.stats_WNC) <- atlases 
#saveRDS(age.max.stats_WNC, "/cbica/projects/network_replication/output/PNC/WNC/GAM/age.max.stats_WNC.RData")
...
age.max.stats_GBC <- readRDS("/cbica/projects/network_replication/output/PNC/GBC/GAM/age.max.stats_GBC.RData")
age.max.stats_BNC <- readRDS("/cbica/projects/network_replication/output/PNC/BNC/GAM/age.max.stats_BNC.RData")
age.max.stats_WNC <- readRDS("/cbica/projects/network_replication/output/PNC/WNC/GAM/age.max.stats_WNC.RData")
...
#saveRDS(cor.credible.intervals_GBC, "/cbica/projects/network_replication/output/PNC/GBC/GAM/cor.credible.intervals_GBC.RData")
max(abs(cor.credible.intervals_GBC$schaefer200$SA.correlation))

cor.credible.intervals_GBC$schaefer200[c(which(cor.credible.intervals_GBC$schaefer200$SA.correlation < -0.7)),]

cor.credible.intervals_BNC <- lapply(atlases, function_cor.credible.intervals, "BNC")
names(cor.credible.intervals_BNC) <- atlases
#saveRDS(cor.credible.intervals_BNC, "/cbica/projects/network_replication/output/PNC/BNC/GAM/cor.credible.intervals_BNC.RData")

 

cor.credible.intervals_WNC <- lapply(atlases, function_cor.credible.intervals, "WNC")
names(cor.credible.intervals_WNC) <- atlases
#saveRDS(cor.credible.intervals_WNC, "/cbica/projects/network_replication/output/PNC/WNC/GAM/cor.credible.intervals_WNC.RData")

```

 
```{r load_cor.credible.intervals, include=FALSE}
cor.credible.intervals_GBC <- readRDS("/cbica/projects/network_replication/output/PNC/GBC/GAM/cor.credible.intervals_GBC.RData")
 
cor.credible.intervals_BNC <- readRDS("/cbica/projects/network_replication/output/PNC/BNC/GAM/cor.credible.intervals_BNC.RData")

cor.credible.intervals_WNC <- readRDS("/cbica/projects/network_replication/output/PNC/WNC/GAM/cor.credible.intervals_WNC.RData")

cor.credible.intervals_GBC$schaefer200
```
...
#ggsave("/Users/audluo/Library/CloudStorage/Box-Box/Box_PhD_Land/PennLINC/network_replication/Figures/OHBM_2023/GBC_schaefer200_devCorr.svg", dpi=300, width = 4, height = 2)
...
for(i in c(1:length(atlases))){
  ...
  assign(gam.edge.age, readRDS(sprintf("/cbica/projects/network_replication/output/%1$s/sensitivity_analyses/%2$s/GAM/GAMresults.%2$s.age.%3$s.RData", dataset, "edge", atlases[i])))
