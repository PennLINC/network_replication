...
3) include passing T1 QC: N=1401, 4102 scans
4) include meanFD < 0.3: N=1270, 3374 scans
5) include scans with at least 7 minutes of scan time: N= 1213,  3317 scans (final sample), 647 females.
...
5) include scans with at least 6 minutes of rest-only scan time (lowered threshold due to fewer total scans available after selecting for rest-only): N= 1000, 1061 scans, 550 females 
...
T1_QA <- read.csv("/cbica/projects/network_replication/input/PNC/sample_selection/T1_QA_PNC_NathaliaEsper_20230630.csv")
T1_QA <- T1_QA %>% dplyr::rename(sub=ID)  
T1_QA_PNC <- T1_QA[which(T1_QA$sub %in% collated_PNC.xcp$sub), ]
...
T1_QA_PNC <- T1_QA_PNC %>% mutate(T1_exclude = ifelse(tri.nary_score_without_rater.2==0, 1, 0))
collated_PNC.xcp_T1Exclusion <- merge(collated_PNC.xcp_medExclusion, dplyr::select(T1_QA_PNC, sub, tri.nary_score_without_rater.2, T1_exclude), by = "sub")
collated_PNC.xcp_T1Exclusion <- collated_PNC.xcp_T1Exclusion[-c(which(collated_PNC.xcp_T1Exclusion$T1_exclude==1)),]

length(unique(collated_PNC.xcp_T1Exclusion$sub)) # N=1401 after excluding participants with failed T1
nrow(collated_PNC.xcp_T1Exclusion) # 4102 scans


collated_PNC.xcp_headMotion <- collated_PNC.xcp_T1Exclusion[c(which(collated_PNC.xcp_T1Exclusion$meanFD < 0.3)),]
length(unique(collated_PNC.xcp_headMotion$sub)) # N=1270 after excluding meanFD >= 0.3  
nrow(collated_PNC.xcp_headMotion) #3374 scans  
 
write.csv(collated_PNC.xcp_headMotion, "/cbica/projects/network_replication/input/PNC/sample_selection/collated_PNC.xcp_headMotion_20230629.csv")
```
...
length(unique(fMRIinclude_CUBIDS$sub)) # N=1213  
nrow(fMRIinclude_CUBIDS) # 3317 scans
...
# saveRDS(PNC_FinalSample_CUBIDS, "/cbica/projects/network_replication/input/PNC/sample_selection/PNC_FinalSample_withCUBIDS_20230103.RData") # OLD
...
length(which(final_dem_df$sex=="3")) # 647 females 
...
mean(final_dem_df$age) # mean = 15.4, SD = 3.4

write.csv(final_dem_df, "/cbica/projects/network_replication/input/PNC/sample_selection/PNC_demographics_finalsample_20230629.csv")
...
length(unique(file_paths_FINAL$sub_task)) # 3317 scans 
length(unique(file_paths_FINAL$subject)) #N=1213
length(unique(file_paths_restOnly_FINAL$sub_task)) # 1069 scans
...
PNC_qc_filenames_atlases_restOnly <- readRDS("/cbica/projects/network_replication/input/PNC/sample_selection/PNC_FinalSample_withCUBIDS_CIFTI_20230629.RData")
...
length(which(fMRIinclude_CUBIDS_ScanTime_restOnly$ScanTime_Total >= 6)) #1000
...
dim(PNC_qc_filenames_atlases_restOnly_final[[1]]) #1061 scans
...
length(unique(final_dem_restOnly_df$sub)) # N=1000
length(which(final_dem_restOnly_df$sex==2)) #550

write.csv(final_dem_restOnly_df, "/cbica/projects/network_replication/input/PNC/sample_selection/PNC_demographics_finalsample_restOnly_20230629.csv")
