...
PNC_CIFTI_summary  <- readRDS("/cbica/projects/network_replication/input/PNC/sample_selection/PNC_FinalSample_restOnly_filenames.RData") 
participants <- unique(PNC_CIFTI_summary[[1]]$sub) 

# paths to all the cifti files (rest for each atlas)
...
listCifti <- function(subject, list_filepath, ciftiFiles_df){
  ...
  saveRDS(cifti_list, paste0(cifti_lists_dir, subject, "_timeseriesRestOnly.RData"))
