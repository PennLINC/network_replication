...
# @param metric A character string, name of connectivity metric ("GBC")
...
make_connMetricsDemog <- function(atlas_network, metric, demog_csv, subj_id, dataset, conn_type){
  subxparcel.matrix <- read.csv(sprintf("%1$s%2$s_subxparcel_matrix_%3$s_%4$s.csv", metric_output_dir, metric, atlas_network, conn_type))
  print(paste(atlas_network, metric, "file loaded"))
  connMetricsDemog_df <- merge(subxparcel.matrix, demog_csv, by=subj_id)
  write.csv(connMetricsDemog_df, sprintf("%1$s%2$s_%3$s_demographics_finalsample_%4$s.csv", metric_output_dir, metric, atlas_network, conn_type), row.names = F)
...
metric = "GBC"
GBC.schaefer200_absvalue <- make_connMetricsDemog("schaefer200", metric, demographics, "subject", dataset, "absvalue")
GBC.schaefer200_thresholded <- make_connMetricsDemog("schaefer200", metric, demographics, "subject", dataset, "thresholded")
 

# fit GAMs 
gam.GBC.age.schaefer200.absvalue <- fitGAMs(schaefer200.parcel.labels, "GBC", "schaefer200", "", dataset, "absvalue")
print("GAMs fit for absvalue")

gam.GBC.age.schaefer200.thresholded <- fitGAMs(schaefer200.parcel.labels, "GBC", "schaefer200", "", dataset, "thresholded")
print("GAMs fit for thresholded")
