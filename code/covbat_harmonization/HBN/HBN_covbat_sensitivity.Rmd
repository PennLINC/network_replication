...
```{r load_subxparcel}
participants_restOnly <- read.csv("/cbica/projects/network_replication/input/HBN/sample_selection/HBN_demographics_finalsample_restOnly.csv")
...
age_vec <- participants_restOnly$age 
...
covar_df <- dplyr::rename(covar_df, sub=...1,
                          ...
                          sex = ...3,
                          meanFD_avgSes = ...4)
#batch <- participants_restOnly$ses 
 
# Harmonize GBC
data.harmonized_GBC <- covfam(data=GBC_subxparcel_schaefer200, bat = as.factor(participants_restOnly$ses), covar = covar_df, gam, y ~ s(age, k=3, fx=T) + as.factor(sex) + as.numeric(meanFD_avgSes))
...
data.harmonized_networkpair <- covfam(data=networkpair_subxnetpair_schaefer200, bat = as.factor(participants$ses), covar = covar_df, gam, y ~ s(age, k=3, fx=T) + as.factor(sex) + as.numeric(meanFD_avgSes))
```
