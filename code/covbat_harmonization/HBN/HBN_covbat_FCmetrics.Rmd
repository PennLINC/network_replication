...
for(i in c(1:length(atlases))){
  ...
  assign(df_name, data.frame(get(df_name)[,-1])) # remove subject column and X
  print(df_name)
} 
...
# Harmonize GBC 
data.harmonized_GBC <- lapply(list(GBC_subxparcel_schaefer200), covfam, bat = as.factor(participants$ses), covar = covar_df, gam, y ~ s(age, k=3, fx=T) + as.factor(sex) + as.numeric(meanFD_avgSes))
names(data.harmonized_GBC) <- atlases_GBC

# Harmonize BNC
data.harmonized_BNC <- lapply(list(BNC_subxparcel_schaefer200x7), covfam, bat = as.factor(participants$ses), covar = covar_df, gam, y ~ s(age, k=3, fx=T) + as.factor(sex) + as.numeric(meanFD_avgSes))
names(data.harmonized_BNC) <- atlases
 
# Harmonize WNC
data.harmonized_WNC <- lapply(list(WNC_subxparcel_schaefer200x7), covfam, bat = as.factor(participants$ses), covar = covar_df, gam, y ~ s(age, k=3, fx=T) + as.factor(sex) + as.numeric(meanFD_avgSes))
