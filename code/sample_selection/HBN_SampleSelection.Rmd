...
collated_HBN.xcp <- read.csv(paste0(qcfile_dir,"HBN_xcp_qc_concat_20230628.csv"))
...
T1_QA <- read.csv(paste0(sample_selection_dir, "T1_QA_HBN_NathaliaEsper_20230630.csv"))
...
demographics <- read.table(paste0(sample_selection_dir,"HBN_Demographics.tsv"), sep="\t", header=TRUE)
...
CUBIDS_summary <- read.csv(paste0(CUBIDS_csv_dir, "HBN_with_orientation_summary_20230214.csv"))
CUBIDS_summary <- CUBIDS_summary[,-c(1:4)]

CUBIDS_files <- read.csv(paste0(CUBIDS_csv_dir, "HBN_with_orientation_files.csv"))
...
write.csv(final_dem_df, paste0(sample_selection_dir,"HBN_demographics_finalsample.csv"), row.names=F)
write.table(final_dem_df$sub, paste0(sample_selection_dir, "HBN_final_subjectlist.txt"), row.names=F, col.names=F)
...
saveRDS(HBN_qc_filenames_atlases, "/cbica/projects/network_replication/input/HBN/sample_selection/HBN_FinalSample_filenames.RData")
...
saveRDS(HBN_qc_filenames_atlases_restOnly_final, "/cbica/projects/network_replication/input/HBN/sample_selection/HBN_FinalSample_filenames_restOnly.RData")
...
write.csv(final_dem_restOnly_df, paste0(sample_selection_dir, "HBN_demographics_finalsample_restOnly.csv"), row.names=F)
write.table(final_dem_restOnly_df$sub, paste0(sample_selection_dir, "HBN_final_subjectlist_restOnly.txt"), row.names = F, col.names=F)
```
