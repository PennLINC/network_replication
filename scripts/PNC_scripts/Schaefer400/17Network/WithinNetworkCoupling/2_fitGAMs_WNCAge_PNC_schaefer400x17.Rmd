---
title: "WNC GAMs: PNC (Schaefer400x17)"
...
```{r} 
WNC_subxparcel_matrix_schaefer400 <- read.csv("/cbica/projects/network_replication/WNC/WNC_subxparcel_matrix_schaefer400x17.csv") #participant x schaefer region WNC spreadsheet
WNC_subxparcel_matrix_schaefer400 <- WNC_subxparcel_matrix_schaefer400[-c(1:7, 24, 690, 691),] # the subjects that should've been removed earlier 
names(WNC_subxparcel_matrix_schaefer400)
col_rm <- as.numeric(levels(as.factor(as.data.frame(which(is.na(WNC_subxparcel_matrix_schaefer400), arr.ind=TRUE))$col)))
WNC_subxparcel_matrix_schaefer400 <- WNC_subxparcel_matrix_schaefer400[, -col_rm]
WNC.schaefer400.pnc <- merge(WNC_subxparcel_matrix_schaefer400, demographics, by="rbcid") #get schaefer WNC data and demographics for final study sample via ID matching
names(WNC.schaefer400.pnc) <- gsub('X17Networks_', "", names(WNC.schaefer400.pnc))
#write.csv(WNC.schaefer400.pnc, "/cbica/projects/network_replication/WNC/WNCschaefer400x17_demographics_finalsample.csv")
...
```{r}
WNC.schaefer400.pnc <- read.csv('/cbica/projects/network_replication/WNC/WNCschaefer400x17_demographics_finalsample.csv')
```
...
schaefer400.parcel.labels$orig_parcelname <- gsub('17Networks_', "", schaefer400.parcel.labels$orig_parcelname)
...
#write.csv(gam.WNC.age.schaefer400, "/cbica/projects/network_replication/WNC/GAM/GAMresults.WNC.age.schaefer400x17.csv", row.names = F, quote = F)
```
 
# load GAMs output
```{r} 
gam.WNC.age.schaefer400 <- read.csv("/cbica/projects/network_replication/WNC/GAM/GAMresults.WNC.age.schaefer400x17.csv")
```
...
# write.csv(gam.smooths.schaefer400,"/cbica/projects/network_replication/WNC/GAM/GAMsmoothfits.WNC.age.schaefer400x17.csv", row.names = F, quote = F)
# write.csv(gam.peaks.schaefer400, "/cbica/projects/network_replication/WNC/GAM/GAMpeaks.WNC.age.schaefer400x17.csv", row.names = F, quote = F)
