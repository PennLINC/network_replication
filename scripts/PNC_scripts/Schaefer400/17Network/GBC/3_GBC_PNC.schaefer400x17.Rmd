---
title: "GBC Developmental Trajectories: PNC (Schaefer400x17)"
...
names(schaefer400.parcel.labels) <- c("orig_parcelname", "label")
...
gam.gbc.age.schaefer400 <- read.csv("/Users/audluo/cbica/projects/network_replication/GBC/GAM/GAMresults.GBC.age.schaefer400x17.csv")

gam.gbc.age.schaefer400 <- gam.gbc.age.schaefer400 %>% select(-label)
gam.gbc.age.schaefer400$label <- schaefer400_SAaxis$label
 

gam.smooths.schaefer400 <- read.csv("/Users/audluo/cbica/projects/network_replication/GBC/GAM/GAMsmoothfits.GBC.age.schaefer400x17.csv")
gam.smooths.schaefer400$orig_parcelname <- gam.smooths.schaefer400$label
 

gam.agepeaks.schaefer400 <- read.csv("/Users/audluo/cbica/projects/network_replication/GBC/GAM/GAMpeaks.GBC.age.schaefer400x17.csv")
gam.agepeaks.schaefer400 <- gam.agepeaks.schaefer400 %>% select(-label)
gam.agepeaks.schaefer400$label <- schaefer400.parcel.labels$label
...
df.list <- list(schaefer400_SAaxis,gam.gbc.age.schaefer400)
...
#perm.id.full <- read.csv("/Users/audluo/cbica/projects/network_replication/software/rotate_parcellation/Schaefer400_perm.csv") # jake's permutations
...
schaefer400.parcel.labels_noMask$label[which(!schaefer400.parcel.labels_noMask$label %in% gbc.axis$label)] # names of removed parcels
...
r <- which(!schaefer400.parcel.labels_noMask$label %in% gbc.axis$label) # indices of removed parcels
existingDF <- gbc.axis  
...
```{r, echo=F, include=F}
plot(schaefer400, show.legend = FALSE)
ggplot() + geom_brain(data = gbc.axis, atlas = schaefer400, aes(fill=as.factor(significant.fdr)), show.legend = NA) +scale_fill_discrete(name = "significant.fdr", labels = c("Not Sig","Sig")) 

 ggseg(.data = gbc.axis, atlas = "schaefer400", mapping=aes(fill=significant.fdr), position = c("stacked")) + theme_void() + scale_fill_discrete(name = "significant.fdr", labels = c("Not Sig","Sig")) # Error: Must request at least one colour from a hue palette.


 ggseg(gbc.axis, atlas = "schaefer400", mapping=aes(fill=significant.fdr), position = "stacked") + scale_fill_gradientn(colours= c("#053061", "#67001F")) + theme_void()
...
```{r, echo=F}
library(ggpubr)
# include all data points in figure
cor.test(gbc.axis$GAM.age.AdjRsq, as.numeric(gbc.axis$SA.axis_rank), method=c("spearman"), exact=F)

gbc.axis <- gbc.axis %>% mutate(SA.axis_rank_signif = ifelse(significant.fdr == 1, SA.axis_rank, NA))

r_text <- expression(paste(italic("r"), "= -0.57, ", italic(p[spin]), "< 0.0001"))
 
## SA axis average rank vs. age effect
(schaefer400_SA.AgeEffect <- ggplot(gbc.axis, aes(x=SA.axis_rank, y=GAM.age.AdjRsq, fill = SA.axis_rank_signif)) + geom_point(color = "white", shape = 21, size=2) +
...
annotate(geom="text", x=290, y=0.06, label=r_text, color="black"))

#ggsave(schaefer400_SA.AgeEffect, file="/cbica/projects/network_replication/adapted_Rscripts/Figures/Schaefer400/GBC/schaefer400x17_SA.AgeEffect.pdf", width = 5, height=4, dpi=300)



# exclude the non-significant FDR corrected data points to calculate the correlation
...
panel.background=element_blank(),
legend.position = "right") +
annotate(geom="text", x=300, y=0.06, label="r = -0.36, p < 0.0001  ", color="black"))
...
```{r}
class(gam.smooths.schaefer400$SA.axis_rank)
...
(schaefer400_predGBC <- ggplot(gam.smooths.schaefer400,aes(age,fit,group=index)) + 
  geom_line(data = gam.smooths.schaefer400, size=.8, alpha = .6, aes(color=SA.axis_rank)) + scale_color_gradient2(low="#6f1282",high = "goldenrod1", mid = "#EBDAFF", midpoint = median(gbc.axis$SA.axis_rank)) + 
...
#ggsave(schaefer400_predGBC, file="/cbica/projects/network_replication/adapted_Rscripts/Figures/Schaefer400/GBC/schaefer400x17_predGBC.pdf", width = 4, height=4, dpi=300)
...
#ggsave(schaefer400_agepeakR, file="/cbica/projects/network_replication/adapted_Rscripts/Figures/Schaefer400/GBC/schaefer400x17_agepeakR.pdf", width = 6, height=4, dpi=300)
...
(schaefer400_agepeakL <- ggplot() + geom_brain(data=left_gam.agepeaks.schaefer400, 
                      ...
                      hemi = "left") + 
  scale_fill_gradientn(colors = viridis_pal(option="B")(10)) + theme_void())
#ggsave(schaefer400_agepeakL, file="/cbica/projects/network_replication/adapted_Rscripts/Figures/Schaefer400/GBC/schaefer400x17_agepeakL.pdf", width = 6, height=4, dpi=300)
