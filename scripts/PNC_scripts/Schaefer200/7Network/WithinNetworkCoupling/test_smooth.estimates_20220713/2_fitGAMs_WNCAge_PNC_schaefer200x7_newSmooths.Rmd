...
wide_df <- WNC.schaefer200.pnc[, c(3:191, 195, 206, 207)]
...
smooth_fits <- long_df %>% group_by(parcel) %>% do(fit = smooth_estimates(gam(WNC ~ s(age,k=6,fx=F),data=.))) %>% unnest(fit)
...
# plot assocation trajectories only
...
smooth_fits$age <- round(smooth_fits$age, 3)
