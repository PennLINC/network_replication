...
ggsave("/Users/audluo/Library/CloudStorage/Box-Box/Box_PhD_Land/PennLINC/network_replication/Writing/Manuscript/Figures/Figures_Illustrator/Fig2/Fig2_corrPlot_lower_noLabel.pdf", dpi=300, width = 5, height = 5)
...
ggsave("/Users/audluo/Library/CloudStorage/Box-Box/Box_PhD_Land/PennLINC/network_replication/Writing/Manuscript/Figures/Figures_Illustrator/Fig2/Fig2_ageSurface_colorbar.pdf", dpi=1200, width = 5, height = 5)
...
# save a version for the colorbar
make_surface_plot_colorbar <- function(dataset) {
  double_age_SA.diff <- get(paste0("double_age_SA.diff_", dataset))
  g2 <- gam(GAM.age.AdjRsq ~ te(parcel2.SA.vec,parcel1.SA.vec,k=3),data = double_age_SA.diff)
...
surfacePlot_colorbar <- make_surface_plot_colorbar("PNC")
surfacePlot_colorbar
ggsave("/Users/audluo/Library/CloudStorage/Box-Box/Box_PhD_Land/PennLINC/network_replication/Writing/Manuscript/Figures/Figures_Illustrator/Fig7/Fig7_colorbar.pdf", dpi=1200, width = 5, height = 5)
...
```{r}
library(gganimate)
(fitted_map1 <- ggplot() + geom_brain(data=fitted_df_PNC, 
                                            atlas=schaefer17_200, 
                                            mapping=aes(fill=fitted), 
                                            show.legend=TRUE, 
                                            position = position_brain(hemi ~ side)) + 
       paletteer::scale_fill_paletteer_c("grDevices::RdYlBu", limits = c(min(fitted_df_PNC$fitted),max(fitted_df_PNC$fitted)), 
                           values=rescale(c(min(fitted_df_PNC$fitted),median(fitted_df_PNC$fitted),max(fitted_df_PNC$fitted)),  oob=squish)) + theme_void())

 
(fitted_map <- ggplot() + geom_brain(data=fitted_df_PNC, 
                                            atlas=schaefer17_200, 
                                            mapping=aes(fill=fitted), 
                                            show.legend=TRUE, 
                                            position = position_brain(hemi ~ side)) + 
       scale_fill_gradient2(low = "goldenrod1", mid = "white", high = "#6f1282", guide = "colourbar", aesthetics = "fill", name = NULL, midpoint = median(fitted_df_PNC$fitted), oob=squish) + theme_void())


unique(fitted_df_PNC$age)
...
mygif <- animate(fitted_map_SO.COOL, width=1600, height=800, fps=20, renderer=gifski_renderer(loop=TRUE))

anim_save(filename="/Users/audluo/Library/CloudStorage/Box-Box/Box_PhD_Land/PennLINC/network_replication/Writing/Manuscript/Figures/fitted_map_Val.gif", mygif)
```
