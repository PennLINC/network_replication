...
make_surface_plot_datasets <- function(dataset) {
  ...
  surface.plot.fig <- gg_tensor(g2)+theme_classic() +
    theme(
      ...
      axis.line = element_line(color = "black"),
      axis.text=element_text(size=24, color = "black")) + 
   scale_fill_gradientn(colors= c("#8a0f63", "#FFFFFF","#f4b100"), limits = c(-0.011,0.019), oob=squish, 
                           values=rescale(c(-0.011,0,0.019))) +
    xlab("Parcel SA Rank") + 
    ylab("Parcel SA Rank") + 
    geom_vline(xintercept = mean(range(double_age_SA.diff$parcel1.SA.vec)),
...
#surfacePlot_PNC
#ggsave("/Users/audluo/Library/CloudStorage/Box-Box/Box_PhD_Land/PennLINC/network_replication/Writing/Manuscript/Figures/Figures_Illustrator/Fig7/colorbar.pdf", dpi=1200, width = 5, height = 5)
